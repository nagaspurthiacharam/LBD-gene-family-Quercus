...
| 9 | `QrLBD_gene_count_matrix.csv` | CSV | 33,247 genes x 11 libs | extract_QrLBD_counts.R, Heatmap_zscore_QrLBD.R |
|10 | `QsLBD_gene_count_matrix.csv` | CSV | 43,163 genes x 5 libs | Heatmap_zscore_QsLBD.R |
...
## 9. `QrLBD_gene_count_matrix.csv` (33,247 x 11)

StringTie / `prepDE.py3` whole-genome gene-level count matrix for *Quercus rubra*. Rows are NCBI RefSeq locus tags (`gene-RGQ29_NNNNNN`); columns are 11 SRA libraries from BioProject **PRJNA273270** (six tissues x two parental genotypes; one tissue is single-genotype). The R scripts join this matrix to `QrLBD_gene_catalogue.csv` on `gene_id`, subset to the 42 primary QrLBDs, and feed the result into the expression heatmap pipeline.

## 10. `QsLBD_gene_count_matrix.csv` (43,163 x 5)

Same format as item 9 but for *Quercus suber*. Rows are LOC IDs (`LOC112XXXXXX` / `LOC136XXXXXX`); columns are five SRA libraries from BioProject **PRJNA392919** (one library per tissue: phellem, xylem, leaf, inner bark, pollen).
